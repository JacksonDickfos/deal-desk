...
export default function DealCard({ deal, onDealUpdate }: DealCardProps) {
  ...
  return (
    <>
      <div 
        className={`rounded-lg border border-gray-200 pt-3 pb-2 pl-3 pr-4 shadow-sm hover:shadow-md transition-shadow cursor-pointer w-[280px] ${
          deal.stage === 'Won' ? 'bg-green-50' : 
          deal.stage === 'Lost' ? 'bg-white' :
          deal.stage === 'Closing' ? 'bg-app-purple/10' :
          'bg-blue-50'
        }`}
      ...
      >
        <div className="flex items-start justify-between mb-1.5">
          <div className="flex-grow min-w-0 pr-2">
            <h3 className="text-base font-semibold text-gray-900 truncate leading-tight mb-1">
              {deal.company}
            </h3>
